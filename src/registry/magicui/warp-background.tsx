...
const Beam = ({
  ...
  // إذا تم تمرير ألوان مخصصة، استخدمها، وإلا استخدم الألوان العشوائية
  let backgroundGradient;
  if (color) {
    if (Array.isArray(color)) {
      // إذا كانت مصفوفة ألوان، اختر لون عشوائي منها
      const randomColor = color[Math.floor(Math.random() * color.length)];
      backgroundGradient = `linear-gradient(${randomColor}, transparent)`;
    } else {
      // إذا كان لون واحد
      backgroundGradient = `linear-gradient(${color}, transparent)`;
    }
  } else {
    // الألوان العشوائية الافتراضية
    const hue = Math.floor(Math.random() * 360);
    backgroundGradient = `linear-gradient(hsl(${hue} 80% 60%), transparent)`;
  }
